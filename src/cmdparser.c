...
static void default_doc_gen_options_handler(FILE *fp, cmdp_option_st *options)
{
    ...
    */
    ...
    for (i = 0; i < count; i++, n++)
    {
        ...
        remain_prefix = HELP_PREFIX_LEN - pos;
        if (remain_prefix >= 2)
        {
            while (remain_prefix--)
            {
...
#define CYAN(STRING)  "\x1b[36m" STRING "\x1b[0m"
    ...
    count = nonzero_countof(cmdp->sub_commands, sizeof(cmdp_command_st *));
    for (i = 0; i < count; i++)
    {
        ...
        cmd_name_len = strlen(command_name) + strlen(p->name) + 2;
        cmd_name      = (char *)malloc(cmd_name_len);
...
static cmdp_command_st *find_command(char *command_name, cmdp_command_st **commands, int count)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        {
            variant_count = nonzero_countof(p->variant_cmds, sizeof(char *));
...
#define __ERROR_HANDLER(_type, _c, _s)                                                                                 \
    ...
    for (; arg_index < argc; arg_index++)
    {
        ...
        if (cur_len >= 2 && cur_arg[0] == '-' && cur_arg[1] != '-')
        {
            ...
            eval_arg      = NULL;
            cur_ch         = '\0';
            for (ch_idx = 1; ch_idx < cur_len; ch_idx++)
            {
                ...
                find = find_option(cur_ch, 0, cmdp);
                if (find == NULL)
                {
        ...
        else if (cur_len >= 3 && cur_arg[0] == '-' && cur_arg[1] == '-')
        {
            ...
            find = find_option(0, long_option_real, cmdp);
            if (find == NULL)
            {
                __ERROR_HANDLER(CMDP_ERROR_UNKNOWN_OPTION, 0, long_option_real);
                return _parse_ret_1;
            }
            eval_arg      = (eq_ptr == NULL) ? next_arg : eq_ptr + 1;
            eval_code = eval_option_output(find, eval_arg);
            find->__flag |= __CMDP_OPT_IS_PARSED;
            switch (eval_code)
            {
    ...
    if (cmdp->sub_commands && arg_index < argc)
    {
        ...
        next_parsed   = cmdp_parse_args(argc - arg_index - 1, argv + arg_index + 1, find, ctx, recursive + 1);
        if (next_parsed < 0)
        {
            return next_parsed;
        }
...
static int cmdp_count_parsed_options(cmdp_command_st *cmdp)
{
    ...
    if (cmdp->options == NULL)
    {
     ...
     count         = 0;
    options_count = nonzero_countof(cmdp->options, sizeof(cmdp_option_st));
...
static cmdp_command_st *cmdp_find_parsed_cmd(cmdp_command_st *cmdp)
{
    ...
    commands_count = nonzero_countof(cmdp->sub_commands, sizeof(cmdp_command_st *));
...
static int cmdp_run_callback(int argc, char **argv, cmdp_command_st *cmdp, cmdp_ctx *ctx, int recursive)
{
    ...
    if (cmdp->fn_process != NULL)
    {
        ...
        code = cmdp->fn_process(&fn_process_param);
        show_help     = HAS_FLAG(code, CMDP_ACT_SHOW_HELP);
        switch (code & 0xFFFF)
        {
            case CMDP_ACT_OK:
                if (show_help)
...
int cmdp_run(int argc, char **argv, cmdp_command_st *root_command, cmdp_ctx *ctx)
{
    ...
    parsed = cmdp_parse_args(argc, argv, root_command, ctx, 0);
    if (parsed < 0)
    {
        /*
        -1: return 0
